...
- **批内并行工具 dispatch**（一条 assistant 消息多个 `tool_use` 并发执行）——另一条并行轴，侵入大（写冲突、gate 串行化、结果对齐），本方案明确不做。
...
| 6 | 批内并行工具 dispatch | **不做**（范围外，§8） | 写冲突 / gate 串行化 / 结果顺序三个硬问题，另案 |
...
- **批内并行工具 dispatch**（一条 assistant 消息多个 `tool_use` 并发）——独立 tech-design。三个硬问题：① 写冲突（并发 `edit_file` 同文件 / `terminal` 改 cwd）需并发策略；② 多工具抢 tui §6 的单权限模态需 Ask 排队；③ `tool_result` blocks 须按 `tool_use_id` 对齐而非完成序。
