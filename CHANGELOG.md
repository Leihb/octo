...
- **Streaming responses** — `provider.StreamingProvider` adds `SendStream(ctx, req, onChunk)` alongside the buffered `Send`. Both `anthropic` and `openai` packages implement it (SSE aggregators native to each protocol — `content_block_delta` / `message_delta` for Anthropic, `chat.completion.chunk` for OpenAI, with `[DONE]` sentinel tolerated-or-omitted). Agent layer mirrors with `agent.StreamingSender` and `Agent.TurnStream`, falling back to the buffered path when the provider doesn't implement the streaming interface. `octo chat` now streams by default; pass `--stream=false` to opt back into buffered output. Compatibility tradeoff: OpenAI streaming does not send `stream_options.include_usage=true` (third-party servers diverge on support), so streamed turns typically report `input_tokens=0`/`output_tokens=0`.
- **Interactive REPL + session persistence (M3, closes #35)** — `octo chat` with no message argument enters an interactive multi-turn REPL (`you> ` prompt, streaming output, auto-save after every turn). Sessions are stored as JSON in `~/.octo/sessions/<YYYYMMDD-HHMMSS>.json`. Resume with `octo chat -c <id>`, list with `octo chat --list-sessions`, opt out of saving with `--no-save`. Slash commands inside the REPL: `/help`, `/cost` (token + USD estimate), `/save`, `/sessions`, `/exit`, `/quit`. Token accumulation and per-model pricing added to `Agent` (`SessionTokens()` / `SessionCostUSD()`). `run()` and `runChat()` now accept an `io.Reader` stdin parameter so REPL tests are fully deterministic without spawning subprocesses.
